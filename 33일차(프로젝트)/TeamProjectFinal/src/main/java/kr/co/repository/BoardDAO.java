...
public class BoardDAO {
	...
	public SearchPageTO<BoardDTO> list(int curPage, String criteria, String keyword) {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new BoardDTO(num, bid, title, null, 0, writeday, readcnt, 0, 0, repIndent));
			}
			pt.setList(list);
	...
	public void update(BoardDTO dto) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE tbl_board SET bid = ?, title=?, "
	...
	private BoardDTO getOrgBoard(Connection conn, int orgNum) {
		...
		try {
			...
			if (rs.next()) {
				...
				orgDto = new BoardDTO(orgNum, null, null, null, 0, null, 0, repRoot, repStep, repIndent);
